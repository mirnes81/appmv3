 ...
 * Version ADMIN - Accessible via session Dolibarr normale
 */
...
if (!$id) {
    print '<h2>Diagnostic Fichiers Planning</h2>';
    print '<p>Usage: <code>?id=XXXXX</code> (ID de l\'événement planning)</p>';
    print '<p>Exemple: <code>?id=74049</code></p>';
    exit;
}
...
if ($res_ecm) {
    while ($file = $db->fetch_object($res_ecm)) {
        $file_data = [
            'rowid' => isset($file->rowid) ? $file->rowid : null,
            'label' => isset($file->label) ? $file->label : 'N/A',
            'filename' => isset($file->filename) ? $file->filename : 'N/A',
            'filepath' => isset($file->filepath) ? $file->filepath : 'N/A',
            'fullpath_orig' => isset($file->fullpath_orig) ? $file->fullpath_orig : 'N/A',
        ];
                    ...
                    <?php if (!empty($step['data']['files'])): ?>
                        <div class="file-list">
                            <?php foreach ($step['data']['files'] as $file): ?>
                                <div class="file-item">
                                    <strong><?php echo htmlspecialchars($file['filename'] != 'N/A' ? $file['filename'] : $file['label']); ?></strong>
                                    <div class="data-item">
                                        <div class="data-label">Chemin relatif:</div>
                                        <div class="data-value"><?php echo htmlspecialchars($file['filepath']); ?></div>
                                    </div>
